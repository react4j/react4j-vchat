...
task 'site:build' do
  project = Buildr.project('react4j-vchat')
  rm_rf SITE_DIR
  mkdir_p SITE_DIR

  cp_r Dir["#{project._('docs')}/*"], SITE_DIR

  %w(react4j.vchat.VideoChatDev react4j.vchat.VideoChatProd).each do |dir|
...
task 'site:deploy' => ['site:build'] do
  ...
  in_dir(local_dir) do
    message ="Update Video Chat website#{travis_build_number.nil? ? '' : " - Travis build: #{travis_build_number}"}"

    rm_rf "#{local_dir}/vchat"
    cp_r "#{SITE_DIR}/vchat", "#{local_dir}/vchat"
    sh 'git add . -f'
